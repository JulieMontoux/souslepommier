...
import type { ClotureApercu } from '@/types/cloture'
import type { PrismaClient } from '@/generated/prisma/client'

export async function computeClotureApercu(db: PrismaClient, date: Date): Promise<ClotureApercu> {
  ...
  // Totaux par mode paiement (net = montant - renduMonnaie pour espèces)
  let totalEspeces = 0,
    totalCB = 0,
    totalCheque = 0,
    totalVirement = 0,
  ...
  const vendeurMap = new Map<
    string,
    { prenom: string; nom: string; nbVentes: number; totalHT: number; totalTTC: number }
  >()
  for (const v of finalisees) {
  ...
  for (const v of finalisees) {
    for (const l of v.lignes) {
      ...
      if (cur) {
        ...
        tvaMap.set(taux, { baseHT: Number(l.montantHT), montantTVA: Number(l.montantTVA) })
      }
    }
  }

  // Recap produits (top 10 par CA HT)
  const produitMap = new Map<string, { nom: string; qteTotal: number; montantHT: number }>()
  for (const v of finalisees) {
    for (const l of v.lignes) {
  ...
  const totalHT = roundFiscal(finalisees.reduce((s, v) => s + Number(v.totalHT), 0))
  const totalTVA = roundFiscal(finalisees.reduce((s, v) => s + Number(v.totalTVA), 0))
  const totalTTC = roundFiscal(finalisees.reduce((s, v) => s + Number(v.totalTTC), 0))

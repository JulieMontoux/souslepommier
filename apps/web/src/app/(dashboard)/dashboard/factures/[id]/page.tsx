...
export const metadata = { title: 'Facture — Sous le Pommier' }
...
export default async function FactureDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [facture, avoir] = await Promise.all([
    ...
    // Find existing avoir for this facture
    prisma.facture.findFirst({
      where: { factureOriginaleId: id },
      select: { id: true },
    }),
  ])
  ...
  const serialised: FactureDetail = {
    id: facture.id,
    ...
    statut: facture.statut as FactureDetail['statut'],
  ...
  return <FactureDetailView facture={serialised} avoirId={avoir?.id} />
}

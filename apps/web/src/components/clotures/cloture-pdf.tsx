...
const s = StyleSheet.create({
  ...
  title: { fontSize: 18, fontFamily: 'Helvetica-Bold', textAlign: 'center', marginBottom: 2 },
  subtitle: { fontSize: 9, textAlign: 'center', color: '#555', marginBottom: 1 },
  company: { fontSize: 10, fontFamily: 'Helvetica-Bold', textAlign: 'center', marginBottom: 8 },
  divider: { borderBottomWidth: 1, borderBottomColor: '#1a1a1a', marginVertical: 8 },
  ...
  totalTTC: { fontSize: 13, fontFamily: 'Helvetica-Bold', textAlign: 'right' },
  hashBox: {
  ...
  hashText: { fontSize: 6.5, fontFamily: 'Courier', color: '#333', wordBreak: 'break-all' },
  footer: {
    position: 'absolute',
    bottom: 20,
    left: 45,
      ...
      <Page size="A4" style={s.page}>
        ...
        {cloture.recapVendeurs.length > 0 && (
          <View style={s.section}>
            ...
            {cloture.recapVendeurs.map((v, i) => (
              <View key={v.vendeurId} style={i % 2 === 0 ? s.tableRow : s.tableRowAlt}>
                <Text style={[s.td, { flex: 3 }]}>
                  {v.prenom} {v.nom}
                </Text>
                <Text style={[s.td, { flex: 1, textAlign: 'right' }]}>{v.nbVentes}</Text>
                <Text style={[s.td, { flex: 2, textAlign: 'right' }]}>{fmt(v.totalHT)}</Text>
                <Text style={[s.td, { flex: 2, textAlign: 'right' }]}>{fmt(v.totalTTC)}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {cloture.recapTVA.length > 0 && (
          <View style={s.section}>
            ...
            <View style={s.tableHeader}>
              ...
              <Text style={[s.th, { flex: 2, textAlign: 'right' }]}>Base HT</Text>
              <Text style={[s.th, { flex: 2, textAlign: 'right' }]}>TVA collectée</Text>
            </View>
            {cloture.recapTVA.map((t, i) => (
              <View key={t.taux} style={i % 2 === 0 ? s.tableRow : s.tableRowAlt}>
                <Text style={[s.td, { flex: 1 }]}>{t.taux}%</Text>
                <Text style={[s.td, { flex: 2, textAlign: 'right' }]}>{fmt(t.baseHT)}</Text>
                <Text style={[s.td, { flex: 2, textAlign: 'right' }]}>{fmt(t.montantTVA)}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {cloture.recapProduits.length > 0 && (
          <View style={s.section}>
            ...
            {cloture.recapProduits.map((p, i) => (
              <View key={p.produitId} style={i % 2 === 0 ? s.tableRow : s.tableRowAlt}>
                <Text style={[s.td, { flex: 4 }]}>{p.nom}</Text>
                <Text style={[s.td, { flex: 1.5, textAlign: 'right' }]}>
                  {p.qteTotal.toFixed(3).replace('.', ',')}
                </Text>
                <Text style={[s.td, { flex: 2, textAlign: 'right' }]}>{fmt(p.montantHT)}</Text>
              </View>
            ))}
          </View>
        )}
            ...
            <View style={s.section}>
              ...
              {cloture.ventesAnnulees.map((v) => (
                <View key={v.numero} style={s.row}>
                  <Text style={s.label}>
                    {v.numero} ({v.vendeurPrenom}){v.motif ? ` — ${v.motif}` : ''}
                  </Text>
                  <Text style={[s.label, { color: '#c00' }]}>{fmt(v.totalTTC)}</Text>
                </View>
              ))}
            </View>
          </>
        ...
        <View style={s.totalBox}>
          ...
          <View style={[s.row, { marginTop: 4 }]}>
            <Text style={{ fontSize: 11, fontFamily: 'Helvetica-Bold' }}>TOTAL TTC</Text>

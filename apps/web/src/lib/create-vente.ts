...
export async function createVente(
  ...
  const lignesComputed = lignes.map((l) => {
    ...
    if (!v) throw venteError(`Variante ${l.varianteProduitId} introuvable`, 404)
    const remise = l.remise ?? 0
    const prixUnitaireHT = Number(v.prixHT)
    const tauxTVA = Number(v.tauxTVA)
    const montantHT = Math.round(prixUnitaireHT * l.qte * (1 - remise / 100) * 10000) / 10000
  ...
  const totalHT = Math.round(lignesComputed.reduce((s, l) => s + l.montantHT, 0) * 10000) / 10000
  const totalTVA = Math.round(lignesComputed.reduce((s, l) => s + l.montantTVA, 0) * 10000) / 10000
  const totalTTC = Math.round(lignesComputed.reduce((s, l) => s + l.montantTTC, 0) * 10000) / 10000
  ...
  return prisma.$transaction(async (tx) => {
    ...
    return tx.vente.create({
      ...
      include: {
        ...
        lignes: {
          include: {
            variante: { include: { produit: { select: { id: true, nom: true } } } },
          },
        },
        paiements: true,
      },

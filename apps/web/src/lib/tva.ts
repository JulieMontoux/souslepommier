/**
 * Calculs TVA conformes à la méthode fiscale française.
 * Arrondi à 2 décimales (centimes), méthode "demi-ajustement" (half-up).
 */
...
/** Récapitulatif TVA d'un ensemble de lignes, groupé par taux. */
export type LigneTVA = { tauxTVA: number; montantHT: number }

export type RecapTVA = {
  taux: number
  baseHT: number
  montantTVA: number
}[]

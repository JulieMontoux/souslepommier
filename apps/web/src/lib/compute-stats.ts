...
import type { DayStats, PeriodStats, YearStats } from '@/types/stats'
...
function aggregateVentes(ventes: VenteWithRelations[]) {
  ...
  const caHT = roundFiscal(finalisees.reduce((s, v) => s + Number(v.totalHT), 0))
  const caTTC = roundFiscal(finalisees.reduce((s, v) => s + Number(v.totalTTC), 0))
...
function buildParHeure(finalisees: VenteWithRelations[]) {
  ...
  return Array.from(map.entries())
    .sort(([a], [b]) => a - b)
    .map(([heure, v]) => ({ heure, nbVentes: v.nbVentes, caTTC: roundFiscal(v.caTTC) }))
}
...
function buildParModePaiement(finalisees: VenteWithRelations[]) {
  ...
  return Array.from(map.entries())
    .map(([mode, v]) => ({ mode, montant: roundFiscal(v.montant), nbVentes: v.nbVentes.size }))
...
function buildParVendeur(finalisees: VenteWithRelations[]) {
  const map = new Map<
    string,
    { prenom: string; nom: string; nbVentes: number; caHT: number; caTTC: number }
  >()
  for (const v of finalisees) {
...
export async function computeDayStats(db: PrismaClient, date: Date): Promise<DayStats> {
...
export async function computePeriodStats(
  ...
  const { finalisees, caHT, caTTC, nbVentes, panierMoyen } = aggregateVentes(ventes)

  const byDay = new Map<string, { caHT: number; caTTC: number; nbVentes: number }>()
  for (const v of finalisees) {
    const key = v.date.toISOString().slice(0, 10)
    ...
    if (cur) {
      ...
      byDay.set(key, { caHT: Number(v.totalHT), caTTC: Number(v.totalTTC), nbVentes: 1 })
    }
  }
...
export async function computeYearStats(db: PrismaClient, year: number): Promise<YearStats> {
  ...
  const byMonth = new Map<number, { caHT: number; caTTC: number; nbVentes: number }>()
  for (const v of finalisees) {
    ...
    if (cur) {
      ...
      byMonth.set(m, { caHT: Number(v.totalHT), caTTC: Number(v.totalTTC), nbVentes: 1 })
    }
  }

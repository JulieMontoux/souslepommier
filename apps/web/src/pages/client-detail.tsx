...
export default function ClientDetailPage() {
  ...
  const { data: client, isLoading } = useQuery<{
    ...
    queryKey: ['clients', id],
    queryFn: async () => {
      const [client, factures] = await Promise.all([
        api.get<{
          id: string
          raisonSociale: string
          siret: string | null
        ...
        api.get<
          { id: string; numero: string; dateEmission: string; totalTTC: number; statut: string }[]
        >(`/clients/${id}/factures`),
      ])
      return { ...client, conditionsPaiement: Number(client.conditionsPaiement), factures }
    },
    enabled: !!id,
  })
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          to={`/dashboard/clients/${id}/modifier`}
